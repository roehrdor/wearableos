...
public class StatisticalFeatureExtraction {
	...
	public double getInterquartileRange() {
		return sortedValues[(int) Math
				.round((sortedValues.length / 4.0) * 3.0 - 1)]
				- sortedValues[(int) Math.round(sortedValues.length / 4.0 - 1)];
	}
	...
	/**
	 * @param values2
	 * @return Returns the Pearson product-moment correlation coefficient.
	 */
	// public double getCorrelationMean(double[] values2) {
	// if(values.length != values2.length) {
	// throw new
	// IllegalStateException("Length of the two arrays does not match");
	// }
	// double mean2 = 0;
	...
	// }
	// return correlation / (getStandardDeviationMean() *
	// standardDeviationMean2);
	// }
	...
	// median2 = (values[values.length / 2] + values[values.length / 2 - 1]) /
	// 2;
	// } else {
	...
	// }
	// return correlation / (getStandardDeviationMedian() *
	// standardDeviationMedian2);
	// }
	...
	public double getCorrelationMean(float[] values2) {
		if (values.length != values2.length) {
			throw new IllegalStateException(
					"Length of the two arrays does not match");
		}
		double mean2 = 0;
		double varianceMean2 = 0;
		double standardDeviationMean2 = 0;
		...
		return correlation
				/ (getStandardDeviationMean() * standardDeviationMean2);
	}
	...
	public double getCorrelationMedian(float[] values2) {
		...
		float[] sortedValues2 = values2.clone();
		Arrays.sort(sortedValues2);
		double median2 = 0;
		double varianceMedian2 = 0;
		double standardDeviationMedian2 = 0;
		if (values2.length % 2 == 0) {
			median2 = (sortedValues2[sortedValues2.length / 2] + sortedValues2[sortedValues2.length / 2 - 1]) / 2;
		} else {
			median2 = sortedValues2[sortedValues2.length / 2];
		}
		for (double value : values2) {
			varianceMedian2 += Math.pow(value - median2, 2);
		}
		...
		return correlation
				/ (getStandardDeviationMedian() * standardDeviationMedian2);
	}
}
